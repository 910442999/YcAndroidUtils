...
public class YcStatusBarUtils {
    ...
    public static void translucent(Activity activity, @ColorInt int colorOn5x) {
        ...
        // 小米和魅族4.4 以上版本支持沉浸式
        if (YcDeviceUtils.isMeizu() || YcDeviceUtils.isMIUI()) {
    ...
    private static boolean isMIUICustomStatusBarLightModeImpl() {
        if (YcDeviceUtils.isMIUIV9() && Build.VERSION.SDK_INT < Build.VERSION_CODES.M){
            return true;
        }
        return YcDeviceUtils.isMIUIV5() || YcDeviceUtils.isMIUIV6() ||
                YcDeviceUtils.isMIUIV7() || YcDeviceUtils.isMIUIV8();
    }
    ...
    public static boolean supportTransclentStatusBar6() {
        return !(YcDeviceUtils.isZUKZ1() || YcDeviceUtils.isZTKC2016());
    }
    ...
    private static void initStatusBarHeight(Context context) {
        ...
        if (YcDeviceUtils.isTablet(context)
                && sStatusbarHeight > YcUtils.dip2px(STATUS_BAR_DEFAULT_HEIGHT_DP)) {
            //状态栏高度大于25dp的平板，状态栏通常在下方
            sStatusbarHeight = 0;
        } else {
            if (sStatusbarHeight <= 0) {
                if (sVirtualDensity == -1) {
                    sStatusbarHeight = YcUtils.px2dip(STATUS_BAR_DEFAULT_HEIGHT_DP);
                } else {
